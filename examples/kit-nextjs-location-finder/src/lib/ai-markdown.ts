...
const PLACEHOLDER_PATTERN = /^(headless-|sxa-|.+placeholder)/i;
const STRUCTURAL_COMPONENT_PATTERN =
  /^(container|partialdesigndynamicplaceholder|globalheader|globalfooter)/i;

function isUsefulText(text: string): boolean {
  const normalized = text.trim();

  if (!normalized) return false;
  if (GUID_PATTERN.test(normalized)) return false;
...
function formatText(raw: unknown): string | undefined {
  ...
  if (!isUsefulText(normalized)) return undefined;
...
function parseRendering(rendering: unknown, headingLevel: number, lines: string[]): void {
  ...
  const shouldEmitComponentHeading =
    componentHeading && headingLevel <= 3 && !STRUCTURAL_COMPONENT_PATTERN.test(componentHeading);

  if (shouldEmitComponentHeading) {
